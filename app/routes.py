...
#     # Fetch bookings for the given email, case-insensitive

#     bookings = Booking.query.filter(func.lower(Booking.client_email) == email.lower()).all()

#     # If no bookings found, return custom error
#     if not bookings:
...
# @bp.route('/book', methods=['POST'])
# def book_class():
#     data = request.get_json(silent=True)  # Try get JSON but don't throw error if none
    
#     if not data:
#         # Try to get from form or query params
#         data = request.form.to_dict() or request.args.to_dict()
    
#     class_id = data.get('class_id')
#     name = data.get('client_name') or data.get('name')  # check both keys to be flexible
